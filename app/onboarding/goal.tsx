...
const GOALS: { value: Goal; title: string; description: string }[] = [
  { value: 'lose', title: 'Perdre du poids', description: 'Déficit calorique pour brûler du gras' },
  { value: 'maintain', title: 'Maintenir', description: 'Garder ton poids actuel' },
  { value: 'gain', title: 'Prendre du poids', description: 'Surplus calorique pour construire du muscle' },
];
...
export default function GoalScreen() {
  ...
  const rates = data.goal === 'lose' ? LOSE_RATES : data.goal === 'gain' ? GAIN_RATES : null;
  const canContinue = data.goal !== null && (data.goal === 'maintain' || data.goalRate !== 0);
  ...
  return (
    <SafeAreaView style={styles.safe}>
      ...
      <ScrollView style={styles.scroll} contentContainerStyle={styles.content}>
        ...
        {GOALS.map((g) => (
          <SelectCard
            ...
            onPress={() => update({ goal: g.value, goalRate: g.value === 'maintain' ? 0 : data.goalRate })}
          />
        ))}
      ...
      <NextButton onPress={() => router.push('/onboarding/body')} disabled={!canContinue} />
    </SafeAreaView>
  );
}
